...
class Spot:
    ...
    def update_neighb(self, grid):
        self.neighbour = []
        if self.row < self.total - 1 and not grid[self.row + 1][self.col].is_barrier(): #CHECK DOWN
            self.neighbour.append(grid[self.row + 1][self.col])

        if self.row > 0 and not grid[self.row - 1][self.col].is_barrier(): #CHECK UP
            self.neighbour.append(grid[self.row - 1][self.col])

        if self.col < self.total - 1 and not grid[self.row][self.col + 1].is_barrier(): #CHECK RIGHT
...
def algor(draw, grid, start, end):
    ...
    while not open_set.empty():
        ...
        for neigh in current.neighbour:
            temp_g_score = g_score[current] + 1

            if temp_g_score < g_score[neigh]:
                came_from[neigh] = current
                g_score[neigh] = temp_g_score
                f_score[neigh] = temp_g_score + h(neigh.get_pos(), end.get_pos())
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and not started:
                    for row in grid:
                        for spot in row:
                            spot.update_neighb(grid)
                    algor(lambda: draw(win, grid, ROWS, width), grid, start, end)
